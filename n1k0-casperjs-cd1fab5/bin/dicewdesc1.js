...
fs.write('D:/ter.txt',JSON.stringify(objArray),'w');
fs.write('D:/ter8.txt','','w');
...
fs.write('D:/ter8.txt',JSON.stringify(objArray),'a');
});
}
...
if (casper.exists(x('//*[@id="jobdesc"]')))
{
var town_info_text2=this.evaluate(function() {
    var nodes = document.querySelectorAll('#jobdesc');
    return [].map.call(nodes, function(node) { // Alternatively: return Array.prototype.map.call(...
      return node.textContent;
    });
  });
...
if (casper.exists(x('//*[@class="error"]')))
{
var town_info_text2='Job is no longer available';
var objArray= town_info_text2.map(function(str) {
str=str.replace(/(\r\n|\n|\r)/gm,"");
    var data = {
...
fs.write('D:/ter.txt',JSON.stringify(objArray),'a');
});
});
casper.then(function(){
//console.log(list.length);
