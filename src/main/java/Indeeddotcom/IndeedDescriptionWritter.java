...
import Monsterdotcom.MonsterDetailgetter;

public class IndeedDescriptionWritter {
	public static void descriptionwriter() throws IOException, WriteException {

		FileInputStream file = new FileInputStream(new File("Indeeddotcom.xls"));
...

		{
			...
			try {
				...
				// Thread.sleep(5000);
				Elements links = doc.select(".bodyText");

				Elements links1 = doc.select("#job_summary");
				Elements links2 = doc.select(".entry-content");
				Elements links3 = doc.select(".container");
				Elements links4 = doc.select(".job-section");
				Elements links5 = doc.select(".job_des");
			...
			System.out.println("Indeed.com Description Written");
		}
		file.close();
		FileOutputStream out = new FileOutputStream(
				new File("Indeeddotcom.xls"));
		workbook.write(out);
		out.close();
