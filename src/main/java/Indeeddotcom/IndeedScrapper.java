...
public class IndeedScrapper extends Thread {
	String searchkeyword;
	String Location;

	public IndeedScrapper(String keyword, String location) {
	...
	public void run() {
		...
		try {
			...
			driver.get("http://www.indeed.com/jobs?q=" + searchKey + "&l=US");
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		try {
			workbook = Workbook.createWorkbook(new File("Indeeddotcom.xls"));
		...
		while (b) {
			// for (int s = 26; s < 28; s++) {
			System.out.println("Indeed.com job detail received");
			try {
				url = "http://www.indeed.com/jobs?q=" + searchKey + "&l="
						+ Location + "&start=" + (pagenumber);
				Document doc = null;
				if (pagenumber == 0) {
					doc = Jsoup
							.connect(
									"http://www.indeed.com/jobs?q=" + searchKey
											+ "&l=" + Location)
					...
					doc = Jsoup
							...
							.ignoreHttpErrors(true).followRedirects(true).get();
					System.out.println("URL: " + url);
					Elements elem = doc.select("span.pn span.np");
				...
				Thread.sleep(15000);
				Elements jobs = doc.select("h2.jobtitle a");
				Elements employer = doc.select("span.company span");
				Elements location = doc.select("span.location span");
				Elements postsed = doc.select("div.result-link-bar span.date");
				...
				for (int i = 0; i < jobs.size(); i++) {
					...
					Label urls = new Label(6, a, jobs.get(i).select("a")
							.attr("href"));
					Label source = new Label(7, a, "Indeed.com");
					Label DesID = new Label(0, a, "IND" + a);
